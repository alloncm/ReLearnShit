...
void Graphics::DrawLine(const Vec2_<float>& point1, const Vec2_<float>& point2, Color c)
{
	float m = (point2.y - point1.y) / (point2.x - point1.x);
	if (std::abs(m) <= 1)
	{
		float b = point1.y - (point1.x * m);
		int startX = std::min(point1.x, point2.x);
		int endX = std::max(point1.x, point2.x);
	...
	else
	{
		...
		float b = point1.x - (point1.y * m);
		int startY = std::min(point1.y, point2.y);
		int endY = std::max(point1.y, point2.y);
...
void Graphics::DrawClosePolyLine(const std::vector<Vec2_<float>>& verticies, Color c)
{
	...
	DrawLine(verticies.back(), verticies.front(), c);
}
